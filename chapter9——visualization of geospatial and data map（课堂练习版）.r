...
#maptools包导入：
china_map1 <- readShapePoly("bou2_4p.shp")#导入CHN_adm文件下的bou2_4p.shp文件
division_data <- china_map1 %>% mutate(id = row.names(.)) %>% .[,c("id","NAME")]    #行政区划层
...
#rgdal包导入:
china_map2 <- ???("D:/Rstudy/CHN_adm/bou2_4p.shp",stringsAsFactors=FALSE)#导入CHN_adm文件下的bou2_4p.shp文件，要求不要自动将非数值型数据转换成有序因子
...
unique(polygons_data$id)
ggplot(polygons_data,aes(???,???,group = group)) +geom_polygon(colour = 'white')#利用polygons_data绘制地理空间点数据
...
#rgdal包读入：
china_map2 <- readOGR('D:/Rstudy/CHN_adm/china.geojson',stringsAsFactors=FALSE)#利用readOGR导入CHN_adm下的china.geojson
