...
china_map <- readOGR("/Users/david/Downloads/吴晓玲/CHN_adm",stringsAsFactors = FALSE)

mydata1   <- china_map??? %>% ???(id = row.names(.)) %>% select(id,NAME) #sp地图对象中的行政区划信息    
china_map1 <- read(china_map)  %>% select(id,long,lat,group,order)         #sp地图对象中的多边形边界点信息
china_map_data <- read(china_map1,mydata1,by ="NAME")                     #合并两个数据框


mydata2   <- read.csv("D:/Rstudy/geshengzhibiao.csv",stringsAsFactors = FALSE)   #读取业务指标数据(csv格式)
...
province_city <- read.csv("D:/Rstudy/chinaprovincecity.csv",stringsAsFactors = FALSE)  #读取省会城市坐标
...
ggplot()+
  ...
  scale_fill_distiller(palette = "BrBG",na.value = "white") +#设置点的颜色（适用于离散型的色板）
...
label_data <- centres %>% ???(centres$NAME???top_10)#在centers表中筛选出top_10中的Name
ggplot()+
  geom_polygon(data = final_data,aes(long,lat,group=group,fill = zhibiao),colour="grey60")+
  geom_point(data = province_city,aes(jd,wd,colour = class,size = zhibiao))+  
  geom_text(data = label_data,aes(x = long,y = lat,label = NAME),family= "myFont") +#修改标签映射参数，只显示指标排行前10的省份
...
ggplot()+
  geom_polygon(data = final_data,aes(long,lat,group=group,fill = zhibiao),colour="grey60")+
  geom_point(data = province_city,aes(jd,wd,colour = class,size = zhibiao))+  
  ggrepel::geom_text_repel(data = province,)
  ggrepel(data = label_data,aes(x = long,y = lat,label = province),family= "myFont") +#调用ggrepel::geom_text_repel解决标签重叠问题
  scale_fill_distiller(palette = "BrBG",na.value = "white") +  
  scale_colour_brewer(palette = "RdBu",na.value = "white") +    
  scale_size_area(max_size = 10) +
  coord_map("polyconic") +
...
windowsFonts(myFont = windowsFont("微软雅黑"))  
china_map <- readOGR("D:/Rstudy/CHN_adm/bou2_4p.shp",stringsAsFactors = FALSE)
...
mydata2   <- read.csv("D:/Rstudy/geshengzhibiao.csv",stringsAsFactors = FALSE)
...
final_data <- ???(china_map_data,mydata2,by = "NAME") %>% select(???)#将china_map_data,mydata2进行左连接，连接后去掉provice一列

#带有各省经纬度数据的辅助数据（可制作散点图、数据标签）
province_city <- read.csv("D:/Rstudy/chinaprovincecity.csv",stringsAsFactors = FALSE)  

# 离散和连续的区别：fill=fau 为离散
ggplot() +
...
###合并分箱后的业务指标等级数据和地理信息数据
final_data <- left_join(china_map_data,mydata2[,c(???,???)],by = "NAME")#从mydata2中筛选出"NAME","zhibiao_q"两列数，通过NAME列将china_map_data,mydata2进行左连接
...
ggplot() +
  geom_polygon(data = final_data, aes(x=long,y=lat,group=group,fill=zhibiao_q),colour="grey65")+
  geom_point(data=province_city,aes(x=jd,y=wd,size = zhibiao, colour = ???),shape=???,alpha=0.6)+#将shape改为20，散点图轮廓颜色由class来确定   
  scale_fill_brewer(palette = "Greens") +  
